...
int main(int argc, char* argv[]) {
    ...
    for(int it = 0; it < NUM_ITERATIONS; it++) {
        ...
        if(dev) {
            dev_start = cpuSecond();
            cudaMemcpy(d_p, d_res, sizeof(Particle)*NUM_PARTICLES, cudaMemcpyHostToDevice);
            step_kernel<<<dim_grid,dim_block>>>(d_p, v_factor, NUM_PARTICLES);
            cudaDeviceSynchronize(); // does cudaMemcpy => implicit synchronization?
    ...
    if(dev) {cudaFree(d_p); cudaFreeHost(d_res);} 

...
{
    // get thread global offset in width and height
    const int index_x = threadIdx.x + blockDim.x * blockIdx.x;
    const int index_y = threadIdx.y + blockDim.y * blockIdx.y;

    // get global offset into 1D array (row width * #row_offset + column_offset)
    const int offset_out = width * index_y + index_x;
    const int offset = offset_out * 3; // input has 3 colors per pixel

    // calculate grayscale value for pixel (idx, idy)
    if(index_x < width && index_y < height) {
        image_out[offset_out] =     image[offset+0] * 0.0722f +
                                    image[offset+1] * 0.7152f +
                                    image[offset+2] * 0.2126f; }
}
...
__device__ float gpu_applyFilter(float *image, int stride, float *matrix, int filter_dim)
{
    ...
    // THIS IMPLEMENTATION IS A DIRECT COPY OF THE cpu_filter ABOVE   
    // I TAKE NO CREDIT FOR IT, INSTEAD REFER TO Wei Der Chien        
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    // get global offset in x and y
    int index_x = blockIdx.x * blockDim.x + threadIdx.x;
    int index_y = blockIdx.y * blockDim.y + threadIdx.y;
    
    // get global 1D-offset
    int offset_t = index_y * width + index_x;

    // get offset in shared array
    int offset_l = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;   
     
    // load from global memory to shared memory
    if (index_x < width && index_y < height) sh_block[offset_l] = image[offset_t];  
    
    // case 1: load values of pixels in two right-most columns
    if(index_x < (width-2) && threadIdx.x+2 >= blockDim.x) sh_block[offset_l+2] = image[offset_t+2];

    // case 2: load values of pixels in two bottom-most rows
    if(index_y < (height-2) && threadIdx.y+2 >= blockDim.y) sh_block[offset_l+2*BLOCK_SIZE_SH] = image[offset_t+2*width];
    
    // case 3: load values of pixels in bottom-right 2x2 
    if(index_x < (width-2) && index_y < (height-2) && threadIdx.x + 2 >= blockDim.x && threadIdx.y + 2 >= blockDim.y)
        sh_block[offset_l + 2*BLOCK_SIZE_SH + 2] = image[offset_t + 2*width + 2];
    
    // wait for all loads to shared memory to complete
    __syncthreads();
    
    // perform gaussian smoothing
    int offset_out   = offset_t + width + 1; 
    if(index_x < (width-2) && index_y < (height-2)) image_out[offset_out] = gpu_applyFilter(&sh_block[offset_l], BLOCK_SIZE_SH, gaussian, 3);
}   
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    __syncthreads();
        ...
        {
            float gx = gpu_applyFilter(&sh_block[offset_l], BLOCK_SIZE_SH, sobel_x, 3);
            float gy = gpu_applyFilter(&sh_block[offset_l], BLOCK_SIZE_SH, sobel_y, 3);
            image_out[offset_out] = sqrtf(gx*gx + gy*gy); 
        }
}

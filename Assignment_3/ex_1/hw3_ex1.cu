...
BMPImage readBMP(char *filename)
{
    ...
    res = fread(g_info, sizeof(BYTE), HEADER_SIZE, file);
    
    printf("Read header. Result: %i\n", res);
    ...
    res = fread(data, sizeof(BYTE), size, file);
    
    printf("Target read size = %i. Read data size= %i\n", size, res);
...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    const int offset_out = width * idy + idx;
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        sh_block[offset_l] = image[offset_t];
        
        // edge cases

        // case 1: load values of pixels in two right-most columns
        if(threadIdx.x+2 >= blockDim.x) {
            sh_block[offset_l+2] = image[offset_t+2];
        }

        // case 2: load values of pixels in two bottom-most rows
        if(threadIdx.y+2 >= blockDim.y) {
            sh_block[offset_l+2*BLOCK_SIZE_SH] = image[offset_t+2*width];
        }

        // case 3: load values of pixels in bottom-right 2x2 
        if(threadIdx.x + 2 >= blockDim.x && threadIdx.y + 2 >= blockDim.y) {
            sh_block[offset_l + 2*BLOCK_SIZE_SH + 2] = image[offset_t + 2*width + 2];
        }
        __syncthreads();

        // perform gaussian smoothing
        int offset   = offset_t + width + 1; 
        image_out[offset] = gpu_applyFilter(&sh_block[offset_l], BLOCK_SIZE_SH, gaussian, 3);
    }  
}   
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        // get offset in block-shared array
        int offset_l = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;
